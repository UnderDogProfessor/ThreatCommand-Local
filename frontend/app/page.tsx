...
type Vulnerability = { cve_id: string; title: string; description: string; severity: string; cvss?: number; kev: boolean; exploitation_evidence?: string; potential_relevance: string; recommended_action: string; source_url?: string; published_at?: string };
type VulnerabilityPage = { items: Vulnerability[]; total: number; offset: number; limit: number; counts: { total: number; live_kev_count: number } };
...
export default function Home() {
  ...
  return <div className="shell">
    ...
    <main><header><form onSubmit={runSearch} className="search"><span>⌕</span><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search local intelligence, CVEs, IOCs…"/><kbd>ENTER</kbd></form><div className="header-actions"><span className="badge local">● LOCAL FIRST</span><button className="badge theme-launch" onClick={() => setActive("Settings")} title="Choose a dashboard theme">◇ THEME</button><button className={`badge mode ${mode}`} onClick={changeMode}>● {mode.toUpperCase()}</button><span className="ollama">✦ AI PROVIDER <small>see Settings</small></span></div></header>
      <section className="workspace">
        <div className="data-status"><div><b>LOCAL INTELLIGENCE WORKSPACE</b><span>Source-reported records stored on this device</span></div><span>{notice}</span></div>
        ...
        {active === "Detection Lifecycle" && <DetectionLifecycleWorkspace detections={detections} openLab={(detectionId) => { window.sessionStorage.setItem("threatcommand_lab_detection", detectionId); setActive("Detection Lab"); }} />}
...
function VulnerabilitiesView({ items }: { items: Dashboard["vulnerabilities"] }) { return <GenericView title="Vulnerability Workspace" copy="Vulnerability records are prioritized for potential relevance. The workspace never claims actual vulnerability, exposure, compromise, or patch status without validated user evidence." children={<div className="table">{items.map((item) => <article key={item.cve_id}><b>{item.cve_id}</b><div><strong>{item.title}</strong><p>{item.potential_relevance}</p></div><span className={item.kev ? "kev" : ""}>{item.kev ? "CISA KEV" : item.severity.toUpperCase()}</span><em>{item.recommended_action}</em></article>)}</div>}/> }
...
function LiveVulnerabilitiesView({ page }: { page: VulnerabilityPage | null }) {
  ...
  const visible = page.items.filter((item) => filter === "all" || item.kev);
  return <GenericView title="Vulnerability Workspace" copy="CISA KEV and other locally stored vulnerability intelligence are listed below. A record is intelligence, not proof of local exposure, compromise, or patch status." children={<>
    <div className="data-totals"><span><b>{page.counts.total.toLocaleString()}</b> local CVE records</span><span><b>{page.counts.live_kev_count.toLocaleString()}</b> CISA KEV records</span><div className="filter-buttons"><button className={filter === "all" ? "selected" : ""} onClick={() => setFilter("all")}>All</button><button className={filter === "live" ? "selected" : ""} onClick={() => setFilter("live")}>CISA KEV</button></div></div>
    <div className="table vulnerability-table">{visible.map((item) => <article key={item.cve_id}><b>{item.cve_id}</b><div><strong>{item.title}</strong><p>{item.description}</p><small>{item.source_url ?? "Local vulnerability record"}</small></div><span className="live">{item.kev ? "CISA KEV" : "CVE"}</span><em>{item.recommended_action}<small>{item.potential_relevance}</small></em></article>)}</div>
...
function InteractiveVulnerabilities({ page, openDetail }: { page: VulnerabilityPage | null; openDetail: (kind: DetailResponse["kind"], id: string) => void }) {
  ...
  return <GenericView title="Vulnerability Workspace" copy="Select any CVE to read its locally stored description, source link, affected products, recommended action, and limitations." children={<>
    <div className="data-totals"><span><b>{page.counts.total.toLocaleString()}</b> local CVE records</span><span><b>{page.counts.live_kev_count.toLocaleString()}</b> CISA KEV records</span><div className="filter-buttons"><button className={filter === "all" ? "selected" : ""} onClick={() => setFilter("all")}>All</button><button className={filter === "live" ? "selected" : ""} onClick={() => setFilter("live")}>CISA KEV</button></div></div>
    <div className="table vulnerability-table">{visible.map((item) => <button className="detail-row" key={item.cve_id} onClick={() => openDetail("vulnerability", item.cve_id)}><b>{item.cve_id}</b><div><strong>{item.title}</strong><p>{item.description}</p><small>{item.source_url ?? "Local vulnerability record"}</small></div><span className="live">{item.kev ? "CISA KEV" : "CVE"}</span><em>{item.recommended_action}<small>{item.potential_relevance}</small></em></button>)}</div>
...
function DetectionLabWorkspace({ detections }: { detections: Detection[] }) {
  ...
  useEffect(() => {
    const preferred = typeof window === "undefined" ? "" : window.sessionStorage.getItem("threatcommand_lab_detection") ?? "";
    setSelectedId((current) => library.some((item) => item.id === current) ? current : (library.some((item) => item.id === preferred) ? preferred : (library[0]?.id ?? "")));
    if (preferred) window.sessionStorage.removeItem("threatcommand_lab_detection");
  }, [detections]);
...
function ThemePreferences() {
  const [theme, setTheme] = useState<"neon-noir" | "signal" | "monochrome">("neon-noir");
  const options = [
    { id: "neon-noir" as const, name: "Night Circuit", detail: "The signature sci-fi look: dark chrome, electric aqua, violet light, and one controlled magenta accent.", swatches: ["#77f7e8", "#ae96ff", "#f06bb6"] },
    { id: "signal" as const, name: "Aerochrome", detail: "A cool, high-clarity command deck with ice blue highlights and a restrained technical feel.", swatches: ["#70d9ff", "#a7c4ff", "#d9e8f6"] },
    { id: "monochrome" as const, name: "Void Chrome", detail: "A quiet graphite mode with silver detail for long investigation and reading sessions.", swatches: ["#f0f4f7", "#96a1ac", "#222933"] },
  ];
  useEffect(() => {
    const stored = window.localStorage.getItem("threatcommand_theme");
    if (stored === "signal" || stored === "monochrome" || stored === "neon-noir") setTheme(stored);
  }, []);
  useEffect(() => {
    document.documentElement.dataset.theme = theme;
    window.localStorage.setItem("threatcommand_theme", theme);
  }, [theme]);
  return <section className="theme-preferences panel"><div className="theme-preferences-copy"><p>INTERFACE THEME</p><h2>Choose your command atmosphere</h2><span>Theme choices are stored only in this browser. They change appearance, not intelligence records, settings, or network behavior.</span></div><div className="theme-options" role="radiogroup" aria-label="Dashboard theme">{options.map((option) => <button key={option.id} className={theme === option.id ? "theme-option selected" : "theme-option"} role="radio" aria-checked={theme === option.id} onClick={() => setTheme(option.id)}><span className="theme-option-top"><span className="theme-swatches">{option.swatches.map((color) => <i key={color} style={{ backgroundColor: color }}/>)}</span>{theme === option.id && <em>ACTIVE</em>}</span><strong>{option.name}</strong><small>{option.detail}</small></button>)}</div></section>;
}
...
function DetectionLifecycleWorkspace({ detections, openLab }: { detections: Detection[]; openLab: (detectionId: string) => void }) {
  const [selectedId, setSelectedId] = useState("");
  const [detail, setDetail] = useState<DetailResponse | null>(null);
  ...
  const persistLifecycle = async (nextMaturity: string, message: string) => {
    if (!selectedId) return;
    const response = await apiFetch(`/api/detections/${selectedId}/maturity`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ maturity: nextMaturity, owner, telemetry_state: telemetryState, validation_notes: validationNotes }) });
    const result = await response.json();
    if (!response.ok) return setNotice(result.detail ?? "The lifecycle state was not saved.");
    setMaturity(nextMaturity); setNotice(message);
    load(selectedId);
  };
  const saveMaturity = async (event: FormEvent) => { event.preventDefault(); await persistLifecycle(maturity, "Lifecycle review saved locally. A revision snapshot was recorded when values changed."); };
  ...
  const results = detail?.validation_results ?? [];
  const hasLabCheck = results.some((result) => result.fixture_name.startsWith("Detection Lab"));
  const hasStaticCheck = results.some((result) => result.fixture_name.startsWith("Built-in static advisory"));
  const hasPassingCheck = results.some((result) => result.outcome === "pass");
  const stages = ["draft", "reviewed", "tested", "validated"];
  const currentStage = ["validated", "tuned", "monitored"].includes(maturity) ? 3 : Math.max(0, stages.indexOf(maturity));
  const checkpointCount = [hasLabCheck, hasStaticCheck, Boolean(owner.trim()), telemetryState === "normalized"].filter(Boolean).length;
  return <GenericView title="Detection Lifecycle" copy="Move a learning detection through a clear local workflow: practice it, review its structure, document the telemetry, and save your decision. Nothing here deploys or modifies another system." children={<>
    {!detections.length ? <div className="panel empty"><b>No detections are available.</b><p>Open Detection Studio after the local library has loaded.</p></div> : <>
      <section className="panel lifecycle-selector"><label>Detection<select value={selectedId} onChange={(event) => setSelectedId(event.target.value)}>{detections.map((item) => <option key={item.id} value={item.id}>{item.title}</option>)}</select></label><button onClick={() => openLab(selectedId)}>Practice in Detection Lab</button></section>
      {detail && <><section className="panel lifecycle-overview"><div><p>LOCAL REVIEW WORKFLOW</p><h2>{detail.record.title}</h2><span>{detail.record.description}</span></div><div className="lifecycle-metrics"><span><b>{results.length}</b> checks saved</span><span><b>{checkpointCount}/4</b> review checkpoints</span></div><div className="lifecycle-stages">{stages.map((stage, index) => <span className={index < currentStage ? "complete" : index === currentStage ? "active" : ""} key={stage}><i>{index + 1}</i>{stage}</span>)}</div></section>
        <section className="lifecycle-grid"><article className="panel lifecycle-next"><p>RECOMMENDED NEXT ACTION</p>{!hasLabCheck ? <><h2>Practice the behavior safely</h2><span>Use the preloaded scenario in Detection Lab. The result is saved back here automatically.</span><button className="button" onClick={() => openLab(selectedId)}>Open Detection Lab</button></> : !hasStaticCheck ? <><h2>Review the rule structure</h2><span>Run the local static advisory to catch basic Sigma or KQL issues without sending or executing a query.</span><button className="button" onClick={runStaticLint}>Run static advisory</button></> : maturity === "draft" ? <><h2>Start the local review</h2><span>Practice and structural review are recorded. Mark this learning template as reviewed when you are ready to document ownership and telemetry.</span><button className="button" onClick={() => persistLifecycle("reviewed", "Marked as locally reviewed. Add an owner, telemetry state, and notes to make the review useful.")}>Mark as reviewed</button></> : maturity === "reviewed" && hasPassingCheck ? <><h2>Record a safe test decision</h2><span>A passing local check exists. Mark it tested only after you have reviewed the observed result and its limitations.</span><button className="button" onClick={() => persistLifecycle("tested", "Marked as locally tested. This remains a learning record, not production coverage.")}>Mark as tested</button></> : <><h2>Keep the review current</h2><span>Update the reviewer, telemetry condition, or notes as your understanding improves. This lifecycle is intentionally local and evidence-led.</span><button className="button" onClick={() => persistLifecycle(maturity, "Lifecycle review saved locally.")}>Save current review</button></>}<small>Local records only — no rule deployment, data collection, blocking, or response action.</small></article>
          <article className="panel lifecycle-checklist"><p>REVIEW CHECKLIST</p><h2>What is documented?</h2><div>{[[hasLabCheck, "Safe lab scenario recorded"], [hasStaticCheck, "Static rule review recorded"], [Boolean(owner.trim()), "Reviewer assigned"], [telemetryState === "normalized", "Telemetry normalized"]].map(([complete, label]) => <span className={complete ? "done" : ""} key={String(label)}><i>{complete ? "✓" : "○"}</i>{String(label)}</span>)}</div><small>A checked item records local learning progress. It is never a claim of real-world detection coverage.</small></article></section>
        <section className="lifecycle-grid"><article className="panel lifecycle-review"><p>UPDATE THE REVIEW</p><h2>Owner, telemetry, and decision</h2><form className="phase-form compact" onSubmit={saveMaturity}><label>Maturity<select value={maturity} onChange={(event) => setMaturity(event.target.value)}><option value="draft">Draft</option><option value="reviewed">Reviewed</option><option value="tested">Tested</option><option value="validated">Validated (local)</option><option value="tuned">Tuned</option><option value="monitored">Monitored</option><option value="retired">Retired</option></select></label><label>Reviewer<input value={owner} onChange={(event) => setOwner(event.target.value)} placeholder="Name or local team"/></label><label>Telemetry<select value={telemetryState} onChange={(event) => setTelemetryState(event.target.value)}><option value="unknown">Unknown</option><option value="unavailable">Unavailable</option><option value="available">Available</option><option value="normalized">Normalized</option></select></label><label>What did you learn?<textarea value={validationNotes} onChange={(event) => setValidationNotes(event.target.value)} placeholder="What was reviewed, what worked, and what still needs verification?"/></label><button>Save local review</button></form></article>
          <article className="panel lifecycle-history"><p>LOCAL EVIDENCE</p><h2>What happened so far</h2>{results.length ? <div>{results.slice(0, 6).map((result) => <article key={`${result.fixture_name}-${result.created_at}`}><b className={result.outcome}>{result.outcome.toUpperCase()}</b><div><strong>{result.fixture_name}</strong><small>{result.evidence || "No supporting note recorded."}</small></div><time>{new Date(result.created_at).toLocaleString()}</time></article>)}</div> : <div className="lifecycle-empty"><b>No evidence yet.</b><span>Start with the Detection Lab; its safe scenario result will appear here.</span></div>}<details><summary>Add a manual safe validation note</summary><form className="phase-form compact" onSubmit={recordValidation}><label>Scenario<input required minLength={2} value={fixture} onChange={(event) => setFixture(event.target.value)} placeholder="Authorized saved event or review scenario"/></label><label>Outcome<select value={outcome} onChange={(event) => setOutcome(event.target.value)}><option value="not_run">Not run</option><option value="pass">Pass</option><option value="fail">Fail</option></select></label><label>Notes<textarea value={validationEvidence} onChange={(event) => setValidationEvidence(event.target.value)} placeholder="Expected versus observed result"/></label><button>Save local validation</button></form></details>{detail.revisions?.length ? <details><summary>View {detail.revisions.length} lifecycle revisions</summary><div className="lifecycle-revisions">{detail.revisions.map((revision) => <p key={`${revision.version}-${revision.created_at}`}><b>v{revision.version}</b> {revision.change_summary} <em>{new Date(revision.created_at).toLocaleString()}</em></p>)}</div></details> : null}</article></section></>}
    </>}{notice && <p className="page-note">{notice}</p>}</>}/>;
}
